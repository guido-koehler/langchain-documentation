...
Run two versions of an agent (e.g., different system prompt, `gpt-5.4` vs `gpt-5.4-mini`, old vs new convention file) against the same dataset and compare scores side-by-side to validate improvements before deploying.
...
| R3 | Azure OpenAI cost overrun | Medium | Medium | Set per-run token budgets; use `gpt-5.4-mini` for low-stakes tasks |
